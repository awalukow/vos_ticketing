...
class HomeController extends Controller
{
    ...
    public function index__usingSeatCount()
    {
        // Temp list table
    ...
    public function index()
    {
        ...
        foreach ($rute_table as $rute) {
            // Total tickets sold and revenue for this route
            $rute->tickets_sold = DB::table('pemesanan')
            ...
            $rute->unpaid_seat = DB::table('pemesanan')
                                ->where('rute_id', $rute->id)
                                ->where('status', 'Belum Bayar')
                                ->where('rowstatus', '>=', 0)
                                ->where('isChurch', '0')
            ...
            // Count unpaid seats for Church
            $rute->unpaid_seat_church = (int) DB::table('pemesanan')
                                        ->where('rute_id', $rute->id)
                                        ->where('status', 'Belum Bayar')
                                        ->where('rowstatus', '>=', 0)
                                        ->where('isChurch', '1')
            ...
            // Count unpaid seats for Fisik
            $rute->unpaid_seat_fisik = (int) DB::table('pemesanan')
                                        ->where('rute_id', $rute->id)
                                        ->where('status', 'Belum Bayar')
                                        ->where('rowstatus', '>=', 0)
                                        ->where('isFisik', '1')
