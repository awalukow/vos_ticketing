    ...
    <div class="modal-dialog">
      <div class="modal-content">
        <div class="modal-header">
          <h5 class="modal-title" id="exampleModalLabel">Tambah Harga Kelas</h5>
          <button
            type="button"
            class="close"
            data-dismiss="modal"
        ...
        <form action="{{ route('rute.store') }}" method="POST">
          ...
          <div class="modal-body">
            ...
            <div class="form-group">
              <label for="transportasi_id">Nama Kelas & Event</label><br>
              <select
              ...
              >
                <option value="" disabled selected>-- Pilih Kelas Event --</option>
                @foreach ($transportasi as $data)
                  <option value="{{ $data->id }}">{{ $data->kode }} - {{ $data->name }} - {{ $data->category->name}}</option>
                @endforeach
              </select>
            </div>
            <div class="form-group">
              <label for="tujuan">Nama Kategori Harga</label>
              <input
                type="text"
                class="form-control"
                id="tujuan"
                name="tujuan"
                placeholder="Nama Kategori Harga"
                required
              />
            </div>
            <div class="form-group" hidden>
              <label for="start">Kode Kursi Start</label>
              <input
                type="text"
                class="form-control"
                id="start"
            ...
            <div class="form-group" hidden>
              <label for="end">Kode Kursi End</label>
              <input
                type="text"
                class="form-control"
                id="end"
            ...
            <div class="form-group">
              <label for="jam">Waktu Event</label>
              <input
                type="time"
                class="form-control"
                id="jam"
